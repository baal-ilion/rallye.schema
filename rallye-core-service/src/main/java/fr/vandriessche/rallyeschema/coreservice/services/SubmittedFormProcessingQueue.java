...
@Service
@Log
public class SubmittedFormProcessingQueue {
	...
	private final ExecutorService executor = Executors.newFixedThreadPool(MAX_CONCURRENT_PROCESSING, runnable -> {
		Thread thread = new Thread(runnable, "submitted-form-processing");
	...
	public void dispatchQueuedFiles() {
		while (availableWorkers.tryAcquire()) {
			SubmittedFormMetadata claimed = claimNext();
			if (claimed == null) {
	...
	public void recoverInterruptedProcessing() {
		...
		var result = mongoTemplate.updateMulti(query, update, SubmittedFormMetadata.class);
		if (result.getModifiedCount() > 0)
			queueUpdatePublisher.publishUpdate();
	}

	private SubmittedFormMetadata claimNext() {
	...
	private void process(String id) {
		try {
			submittedFormService.processQueuedSubmittedForm(id);
			...
			mongoTemplate.updateFirst(query, update, SubmittedFormMetadata.class);
			queueUpdatePublisher.publishUpdate();
		} finally {
			availableWorkers.release();
		}
