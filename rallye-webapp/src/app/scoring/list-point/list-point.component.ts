...
export class ListPointComponent implements OnInit, OnDestroy {
  ...
  readonly challengeOrder = (a: { key: string }, b: { key: string }) => Number(a.key) - Number(b.key);
  ...
  downloadPoints(): void {
    ...
    this.points.forEach(point => {
      Object.values(point.challengePoints || {})
        .sort((left: any, right: any) => Number(left.challenge) - Number(right.challenge))
        .forEach((challengePoint: any) => {
          const questions = challengePoint.questions || [];
          if (!questions.length) {
            rows.push({
              'Équipe': point.team,
              'Épreuve': challengePoint.challenge,
              'Question': '',
              'Points de la question': '',
              'Total de l’épreuve': challengePoint.total,
