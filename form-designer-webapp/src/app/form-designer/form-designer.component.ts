...
import { DesignerBlock, DesignerBlockType, DesignerCorrection, DesignerQuestion, DesignerSection, DesignerChallenge, FORM_PROJECT_SCHEMA_VERSION,
  FormProject } from './form-project.model';
import { FormDesignerApiService, FormDesignDto, GeneratedRecognitionPageDto, QuestionDefinitionDto, QuestionScoringDto,
  RallyConfigurationDto, ChallengeConfigurationDto } from './form-designer-api.service';
...
export class FormDesignerComponent implements OnInit, AfterViewChecked {
  ...
  readonly referenceChallengeId = '__reference_form__';
  ...
  referenceChallenge: DesignerChallenge = this.makeReferenceChallenge();
  activeChallengeId = this.project.challenges[0].id;
  ...
  private rallyConfiguration?: RallyConfigurationDto;
  private readonly requestedChallengeId = new URLSearchParams(window.location.search).get('challengeId');
  private readonly challengeConfigurations = new Map<string, ChallengeConfigurationDto>();
  ...
  get activeChallenge(): DesignerChallenge {
    if (this.isReferenceActive) { return this.referenceChallenge; }
    return this.project.challenges.find(challenge => challenge.id === this.activeChallengeId) || this.project.challenges[0];
  }

  get isReferenceActive(): boolean { return this.activeChallengeId === this.referenceChallengeId; }
  get correctionAppUrl(): string {
    const path = this.isReferenceActive
      ? '/configuration/challenges'
      : `/configuration/challenges/${encodeURIComponent(this.activeChallenge.id)}`;
  ...
  get challengeTitleWidthMm(): number {
    const titleHtml = String(this.activeChallenge.headerTitle || '');
  ...
  get titleBlockHeightMm(): number {
    const textHeight = this.richTextHeightMm(this.activeChallenge.headerTitle || '', 45) + 3;
    return Math.max(this.project.showLogo ? 21 : 18, textHeight);
  }

  isChallengeDigitMarked(position: 0 | 1, digit: number): boolean {
    const challengeNumber = Math.max(0, Math.min(99, Math.trunc(Number(this.activeChallenge.number) || 0)));
    return Number(String(challengeNumber).padStart(2, '0')[position]) === digit;
  }

  get sections(): DesignerSection[] { return this.activeChallenge?.sections || []; }
  ...
  get blocks(): DesignerBlock[] {
    if (!this.activeChallenge.blocks) { this.activeChallenge.blocks = []; }
    return this.activeChallenge.blocks;
  }
  ...
  addChallenge(): void {
    const onlyDraft = this.challengeConfigurations.size === 0 && this.project.challenges.length === 1;
    const nextNumber = Math.max(0, ...this.project.challenges.map(challenge => challenge.number)) + 1;
    const draftChallenge = this.project.challenges[0];
    const number = onlyDraft ? draftChallenge.number : nextNumber;
    this.setSyncState('saving', 'Création de l’épreuve…');
    this.api.createChallenge({ ...this.emptyChallengeConfiguration(number), name: onlyDraft ? draftChallenge.name : 'Nouvelle épreuve' }).subscribe({
      next: value => {
        const challenge = this.makeChallengeFromConfiguration(value);
        this.challengeConfigurations.set(challenge.id, value);
        if (onlyDraft) { this.project.challenges[0] = challenge; }
        else { this.project.challenges.push(challenge); }
  ...
  selectChallenge(id: string): void {
    ...
    if (!this.challengeConfigurations.has(id)) { return; }
    this.api.getChallenge(id).subscribe({
      next: challengeConfiguration => {
        this.challengeConfigurations.set(id, challengeConfiguration);
        if (this.activeChallengeId !== id) { return; }
        this.hydrateDesignerPoints(this.activeChallenge, challengeConfiguration);
        this.refreshPages();
  ...
  deleteActiveChallenge(): void {
    if (!confirm(`Supprimer l'épreuve « ${this.activeChallenge.name} », son formulaire et toutes ses données associées ?`)) {
      return;
    }
    const challengeId = this.activeChallenge.id;
    if (!this.challengeConfigurations.has(challengeId)) {
      this.project.challenges[0] = this.makeChallenge(1);
      this.activeChallengeId = this.project.challenges[0].id;
      this.setSyncState('ready', 'Aucune épreuve enregistrée.');
      this.refreshPages();
      return;
    }
    const index = this.project.challenges.findIndex(challenge => challenge.id === challengeId);
    this.setSyncState('saving', 'Suppression de l’épreuve…');
    this.api.deleteChallenge(challengeId).subscribe({
      next: () => {
        this.project.challenges.splice(index, 1);
        this.challengeConfigurations.delete(challengeId);
        if (!this.project.challenges.length) { this.project.challenges.push(this.makeChallenge(1)); }
        this.selectChallenge(this.project.challenges[Math.max(0, index - 1)].id);
        this.setSyncState('saved', this.challengeConfigurations.size
  ...
  saveActiveChallenge(): void {
    ...
    this.saveChallenges([this.activeChallenge]);
  }
  ...
  createActiveForm(): void {
    if (this.activeChallenge.hasFormDesign) { return; }
    this.activeChallenge.hasFormDesign = true;
    this.activeChallenge.headerTitle = this.activeChallenge.name;
    this.questionEdited();
  ...
  deleteActiveForm(): void {
    ...
    if (!challenge.hasFormDesign || !confirm(`Supprimer le formulaire de « ${challenge.name} » ? L’épreuve sera conservée.`)) {
      return;
    }
    this.setSyncState('saving', 'Suppression du formulaire…');
    this.api.deleteRecognitionPages(challenge.number).pipe(
      switchMap(() => this.api.deleteFormDesign(challenge.id))
    ).subscribe({
      next: () => {
        challenge.hasFormDesign = false;
        challenge.formDesignVersion = undefined;
        challenge.sections = [];
        challenge.blocks = [];
  ...
  private loadSharedConfiguration(): void {
    ...
    forkJoin({
      ...
      challenges: this.api.getChallenges(),
      designs: this.api.getFormDesigns(),
      referenceDesign: this.api.getReferenceFormDesign()
    }).subscribe({
      next: ({ rally, challenges, designs, referenceDesign }) => {
        this.rallyConfiguration = rally;
        this.challengeConfigurations.clear();
        this.publishedDesignerLabels.clear();
        const challengeConfigurations = this.api.challengeItems(challenges)
          .sort((left, right) => left.challenge - right.challenge);
        const designsByChallenge = new Map(designs.map(design => [design.challengeConfigurationId, design]));
        const designerChallenges = challengeConfigurations.map(challengeConfiguration => {
          if (challengeConfiguration.id) { this.challengeConfigurations.set(challengeConfiguration.id, challengeConfiguration); }
          const design = designsByChallenge.get(challengeConfiguration.id);
          if (challengeConfiguration.id) {
            this.publishedDesignerLabels.set(challengeConfiguration.id,
              new Set(design ? this.designerCorrectionEntries(design.content).map(entry => entry.label) : []));
          }
          return this.challengeFromSharedData(challengeConfiguration, design);
        });
        const draft = this.makeChallenge(1);
        this.project = {
          ...
          challenges: designerChallenges.length ? designerChallenges : [draft]
        };
        this.referenceChallenge = referenceDesign?.content
          ? this.clone(referenceDesign.content)
          : this.makeReferenceChallenge();
        ...
        const requestedChallenge = this.requestedChallengeId
          ...
          : undefined;
        this.activeChallengeId = requestedChallenge?.id || this.project.challenges[0].id;
        this.selectedBlockId = this.blocks[0]?.id || '';
        this.importError = '';
        this.refreshPages();
        this.setSyncState('ready', designerChallenges.length
  ...
  private async saveChallenges(challenges: DesignerChallenge[]): Promise<void> {
    const restoreView = await this.freezeVisibleInterface(
      challenges.length === 1 ? 'Enregistrement de l’épreuve' : 'Enregistrement des épreuves');
    this.setSyncState('saving', challenges.length === 1 ? 'Enregistrement de l’épreuve…' : 'Enregistrement des épreuves…');
    try {
      this.validateChallengesBeforeSave(challenges);
      await this.saveRally();
      await this.persistChallenges(challenges);
      if (challenges.some(challenge => challenge.id === this.activeChallenge.id)) { await this.publishActiveRecognition(); }
      this.setSyncState('saved', challenges.length === 1 ? 'Épreuve enregistrée.' : 'Épreuves enregistrées.');
    } catch (error) {
  ...
  private async saveRally(): Promise<void> {
    const current = this.rallyConfiguration || {
    ...
    this.rallyConfiguration = await firstValueFrom(this.api.saveRally({
      ...current,
  ...
  private async persistReferenceForm(): Promise<void> {
    const saved = await firstValueFrom(this.api.saveReferenceFormDesign({
      version: this.referenceChallenge.formDesignVersion,
      schemaVersion: FORM_PROJECT_SCHEMA_VERSION,
      content: this.clone(this.referenceChallenge)
    }));
    this.referenceChallenge.formDesignVersion = saved.version;
    this.referenceChallenge.hasFormDesign = true;
  }

  private async persistChallenges(challenges: DesignerChallenge[]): Promise<void> {
    for (const challenge of challenges) {
      let challengeConfiguration = this.challengeConfigurations.get(challenge.id);
      const payload: ChallengeConfigurationDto = {
        ...(challengeConfiguration || this.emptyChallengeConfiguration(challenge.number)),
        id: challengeConfiguration?.id,
        challenge: challenge.number,
        name: challenge.name
      };
      this.applyDesignerQuestions(payload, challenge);
      delete payload._links;
      const savedChallenge = challengeConfiguration
        ? await firstValueFrom(this.api.updateChallenge(payload))
        : await firstValueFrom(this.api.createChallenge(payload));
      if (!savedChallenge.id) { throw new Error('Le back n’a pas retourné l’identifiant de l’épreuve.'); }
      if (savedChallenge.id !== challenge.id) {
        const oldId = challenge.id;
        challenge.id = savedChallenge.id;
        if (this.activeChallengeId === oldId) { this.activeChallengeId = savedChallenge.id; }
      }
      this.challengeConfigurations.set(savedChallenge.id, savedChallenge);
      challengeConfiguration = savedChallenge;
      if (challenge.hasFormDesign) {
        const content = this.clone(challenge);
        content.formDesignVersion = undefined;
        const savedDesign = await firstValueFrom(this.api.saveFormDesign(savedChallenge.id, {
          version: challenge.formDesignVersion,
          challengeConfigurationId: savedChallenge.id,
          schemaVersion: FORM_PROJECT_SCHEMA_VERSION,
          content
        }));
        challenge.formDesignVersion = savedDesign.version;
        this.publishedDesignerLabels.set(savedChallenge.id,
          new Set(this.designerCorrectionEntries(challenge).map(entry => entry.label)));
      }
    }
  }

  private challengeFromSharedData(challengeConfiguration: ChallengeConfigurationDto, design?: FormDesignDto): DesignerChallenge {
    const challenge = design?.content ? this.clone(design.content) : this.makeChallengeFromConfiguration(challengeConfiguration);
    this.normalizeChallengeDesign(challenge);
    challenge.id = challengeConfiguration.id || challenge.id;
    challenge.number = challengeConfiguration.challenge;
    challenge.name = challengeConfiguration.name;
    if (this.plainText(challenge.headerTitle).trim() !== challengeConfiguration.name.trim()) {
      challenge.headerTitle = challengeConfiguration.name;
    }
    challenge.sections ||= [];
    challenge.blocks ||= [];
    challenge.hasFormDesign = !!design;
    challenge.formDesignVersion = design?.version;
    if (design) { this.hydrateDesignerPoints(challenge, challengeConfiguration); }
    return challenge;
  }

  private makeChallengeFromConfiguration(challengeConfiguration: ChallengeConfigurationDto): DesignerChallenge {
    const challenge = this.makeChallenge(challengeConfiguration.challenge);
    challenge.id = challengeConfiguration.id || challenge.id;
    challenge.name = challengeConfiguration.name;
    challenge.headerTitle = challengeConfiguration.name;
    challenge.hasFormDesign = false;
  ...
  private emptyChallengeConfiguration(number: number): ChallengeConfigurationDto {
    return {
      challenge: number,
  ...
  private designerCorrectionEntries(challenge: DesignerChallenge): Array<{ label: string; points: number }> {
    return (challenge.sections || []).flatMap(section => section.questions.flatMap(question =>
  ...
  private validateDesignerQuestions(challenges: DesignerChallenge[]): void {
    for (const challenge of challenges.filter(item => item.hasFormDesign)) {
      const entries = this.designerCorrectionEntries(challenge);
      const empty = entries.find(entry => !entry.label);
      if (empty) {
        throw new Error(`L’épreuve « ${challenge.name} » contient un bloc de correction sans label.`);
      }
      const labels = new Map<string, string>();
      for (const entry of entries) {
        const normalized = entry.label.toLocaleLowerCase('fr-FR');
        if (labels.has(normalized)) {
          throw new Error(`Le label « ${entry.label} » est utilisé plusieurs fois dans l’épreuve « ${challenge.name} ».`);
        }
      ...
      const challengeConfiguration = this.challengeConfigurations.get(challenge.id);
      for (const entry of entries) {
        if (challengeConfiguration?.questionDefinitions?.[entry.label]?.type === 'PERFORMANCE') {
          throw new Error(`Le label « ${entry.label} » est déjà utilisé par une performance dans l’épreuve « ${challenge.name} ».`);
        }
      }
    }
  }

  private applyDesignerQuestions(challengeConfiguration: ChallengeConfigurationDto, challenge: DesignerChallenge): void {
    if (!challenge.hasFormDesign) { return; }
    const questionDefinitions: Record<string, QuestionDefinitionDto> = { ...(challengeConfiguration.questionDefinitions || {}) };
    const questionScoringByLabel: Record<string, QuestionScoringDto> = { ...(challengeConfiguration.questionScorings || {}) };
    const currentEntries = this.designerCorrectionEntries(challenge);
    const currentLabels = new Set(currentEntries.map(entry => entry.label));
    for (const previousLabel of this.publishedDesignerLabels.get(challenge.id) || []) {
      if (!currentLabels.has(previousLabel)) {
        questionDefinitions[previousLabel] = { name: previousLabel };
        questionScoringByLabel[previousLabel] = { name: previousLabel, point: null };
      }
    }
    for (const entry of currentEntries) {
      const existing = questionDefinitions[entry.label];
      questionDefinitions[entry.label] = {
        name: entry.label,
        type: 'QUESTION',
        managedByOrganizer: existing?.managedByOrganizer ?? false
      };
      questionScoringByLabel[entry.label] = { name: entry.label, point: entry.points };
    }
    challengeConfiguration.questionDefinitions = questionDefinitions;
    challengeConfiguration.questionScorings = questionScoringByLabel;
  }

  private hydrateDesignerPoints(challenge: DesignerChallenge, challengeConfiguration: ChallengeConfigurationDto): void {
    for (const section of challenge.sections || []) {
      for (const question of section.questions) {
        for (const correction of question.corrections) {
          ...
          const sharedPoint = challengeConfiguration.questionScorings?.[label]?.point;
  ...
  private async publishActiveRecognition(): Promise<void> {
    ...
    try {
      ...
      const expectedLabels = (this.isReferenceActive ? [] : this.designerCorrectionEntries(this.activeChallenge)
      ...
      if (!labelsMatch(actualLabels)) {
        ...
        throw new Error(`Le formulaire « ${this.activeChallenge.name} » n’est pas entièrement rendu`
      ...
      if (this.isReferenceActive) {
        ...
        await firstValueFrom(this.api.publishRecognitionPages(this.activeChallenge.number, generatedPages));
        const refreshedChallenge = await firstValueFrom(this.api.getChallenge(this.activeChallenge.id));
        this.challengeConfigurations.set(this.activeChallenge.id, refreshedChallenge);
        this.hydrateDesignerPoints(this.activeChallenge, refreshedChallenge);
      }
    } finally {
  ...
  private validateChallengesBeforeSave(challenges: DesignerChallenge[]): void {
    for (const challenge of this.project.challenges) {
      if (!Number.isInteger(challenge.number) || challenge.number < 1 || challenge.number > 99) {
        throw new Error(`Le numéro de l’épreuve « ${challenge.name || 'sans nom'} » doit être un entier compris entre 1 et 99.`);
      }
      if (!challenge.name.trim()) {
        throw new Error(`L’épreuve n° ${challenge.number} ne peut pas être enregistrée sans titre.`);
      }
    }

    const challengesByNumber = new Map<number, DesignerChallenge[]>();
    this.project.challenges.forEach(challenge => {
      const sameNumber = challengesByNumber.get(challenge.number) || [];
      sameNumber.push(challenge);
      challengesByNumber.set(challenge.number, sameNumber);
    });
    const duplicate = Array.from(challengesByNumber.entries())
      .find(([, matchingChallenges]) => matchingChallenges.length > 1);
    if (duplicate) {
      const [number, matchingChallenges] = duplicate;
      const names = matchingChallenges.map(challenge => `« ${challenge.name} »`).join(' et ');
      throw new Error(`Le numéro d’épreuve ${number} est déjà utilisé par ${names}. Choisissez un numéro différent.`);
    }

    this.validateDesignerQuestions(challenges);
  }
  ...
  private async publishAllRecognitions(): Promise<void> {
    ...
    const challengeIds = [
      ...(allChallengeIds.includes(previousChallengeId) ? [previousChallengeId] : []),
      ...allChallengeIds.filter(challengeId => challengeId !== previousChallengeId)
    ];
    try {
      ...
      this.activeChallengeId = previousChallengeId;
      this.selectedBlockId = previousSelectedBlockId;
      this.pages = [];
      this.changeDetector.detectChanges();
      this.refreshPages();
  ...
  private async generateRecognitionPage(paper: HTMLElement, page: number | undefined,
    ...
    return {
      configuration: {
        challenge: referenceOnly ? undefined : this.activeChallenge.number,
      ...
      imageBase64: canonicalPng.split(',')[1],
...
${referenceOnly ? '' : `        <group name="Questions">\n${xmlQuestions(corrections)}\n        </group>`}
  ...
  private handleSyncError(error: unknown, fallback: string): void {
    ...
    if (/(duplicate|doublon|déjà utilisé|already|e11000)/i.test(normalizedMessage)
        && /(challenge|épreuve)/i.test(normalizedMessage)) {
      this.setSyncState('error',
        `Le numéro d’épreuve ${this.activeChallenge.number} est déjà utilisé. Choisissez un numéro différent.`);
      return;
    }
  ...
  async importWorkbook(files: FileList | null): Promise<void> {
    ...
    try {
      ...
      this.activeChallenge.sourceFileName = file.name;
      this.activeChallenge.hasFormDesign = true;
      this.activeChallenge.sections = sections;
      this.activeChallenge.blocks = sections.map(section => {
        const block = this.makeBlock('section', section.title, section.id);
        block.color = section.color;
        return block;
      });
      this.selectedBlockId = this.activeChallenge.blocks[0]?.id || '';
      this.questionEdited();
    } catch (error) {
  ...
  updateChallengeHeaderTitle(event: Event): void {
    this.updateRichText(this.activeChallenge, 'headerTitle', event);
    this.activeChallenge.name = this.plainText(this.activeChallenge.headerTitle)
      .replace(/\s+/g, ' ').trim();
  }
  ...
  dropBlock(event: CdkDragDrop<DesignerBlock[]>): void {
    ...
    this.activeChallenge.blocks = this.deduplicateBlockTree(this.activeChallenge.blocks ?? []);
    this.synchronizeSectionOrder();
  }
  ...
  async exportPdf(corrected: boolean): Promise<void> {
    if (this.pdfExporting || !this.activeChallenge.hasFormDesign) { return; }
    const restoreView = await this.freezeVisibleInterface(
    ...
    try {
      ...
      const challengeName = this.pdfFileName(`${this.activeChallenge.number} - ${this.activeChallenge.name}`);
      pdf.save(`${challengeName} - Formulaire ${mode}.pdf`);
    } catch (error) {
  ...
  async exportAllChallengePdfs(): Promise<void> {
    if (this.pdfExporting) { return; }
    const challenges = this.project.challenges.filter(challenge => challenge.hasFormDesign)
    ...
    try {
      ...
      for (const challenge of challenges) {
        ...
        this.changeDetector.detectChanges();
        const challengeName = this.pdfFileName(`${challenge.number} - ${challenge.name}`);
        for (const corrected of [false, true]) {
          const pdf = await this.createActivePdf(corrected);
          const mode = corrected ? 'complété' : 'vierge';
          archive.file(`${challengeName} - Formulaire ${mode}.pdf`, pdf.output('arraybuffer'));
        }
      }
  ...
  private async createActivePdf(corrected: boolean): Promise<jsPDF> {
    ...
    if (!papers.length) { throw new Error(`Aucune page A4 pour l’épreuve « ${this.activeChallenge.name} ».`); }
  ...
  private makeProject(): FormProject {
    const firstChallenge = this.makeChallenge(1);
    return {
      kind: 'rallye-form-project',
      schemaVersion: FORM_PROJECT_SCHEMA_VERSION,
  ...
  private makeChallenge(number: number): DesignerChallenge {
    return {
  ...
  private makeReferenceChallenge(): DesignerChallenge {
    return {
      id: this.referenceChallengeId,
      hasFormDesign: true,
      number: 0,
  ...
  private selectContentImage(image: HTMLImageElement): void {
    ...
    if (section?.dataset['sectionId']) {
      ...
      this.selectedContentEditorKey = 'challenge-title';
  ...
  private currentContentImage(): HTMLImageElement | null {
    ...
    } else if (this.selectedContentEditorKey === 'challenge-title') {
  ...
  private normalizeChallengeDesign(challenge: DesignerChallenge): void {
    challenge.sections ||= [];
    challenge.blocks ||= [];
    challenge.blocks = this.deduplicateBlockTree(challenge.blocks);
    ...
    this.flattenBlocks(challenge.blocks).forEach(block => {
  ...
  private synchronizeSectionOrder(): void {
    this.activeChallenge.sections = [...this.sectionsInDocumentOrder()];
    this.refreshPages();
  }
